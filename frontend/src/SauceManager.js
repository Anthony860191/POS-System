 ...
 * Creates a table that allows the user to set inventory amount and add items to the sauce category.
 * @constructor
...
function SauceManager({ lang, mode }) {
    ...
    useEffect(() => {
        ...
        axios.get(`${url}ingredients/?ingr_type=SAUCE`, {signal:controller.signal})
            .then(res => {
                setItems(res.data);
