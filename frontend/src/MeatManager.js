 ...
 * Creates a table that allows the user to set inventory amount and add items to the meat category.
 * @constructor
...
function MeatManager({ lang, mode }) {
    ...
    useEffect(() => {
        axios.get("http://localhost:8000/ingredients/?ingr_type=MEAT")
            .then(res => {
                setItems(res.data);
